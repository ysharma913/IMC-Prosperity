        ...
        for product in state.order_depths.keys():
            ...
            if product in self.last_exp:
                ...
                if self.opposite_signs(slope, last_slope):
                    ...
                    if slope > last_slope:
                        self.do_order(bot_orders = order_depth.sell_orders, operator = operator.lt, max_vol = max_buy, acceptable_price= expected_val_total, trade_made="BUY", product=product, order_lst = orders)
