...
class StaticTrader:
    ...
    limits = {'PEARLS': 20, 'BANANAS': 20, 'COCONUTS': 600, 'PINA_COLADAS': 300, 'BERRIES': 250, 'DIVING_GEAR': 50, "BAGUETTE" : 150, 'DIP': 300, "UKULELE": 70, "PICNIC_BASKET": 70}
...
class MeanReversion:
        ...
        def __init__(self, window_size: int, z_thresh: int, product : str,  ordering_thresh = 1, historical_mean = None):
            self.rolling_window = list()
            self.WINDOW_SIZE = window_size
            if historical_mean:
                self.rolling_window = [historical_mean] * window_size
            self.Z_THRESH = z_thresh
            self.product = product
            self.ordering_thresh = ordering_thresh
        ...
        def make_orders(self, state):
            ...
            if len(self.rolling_window) >= self.WINDOW_SIZE:
                ...
                if z_score < -self.Z_THRESH:
                    buy_prices = StaticTrader.do_order_price(bot_orders = order_depth.sell_orders, operator = operator.lt, max_vol = max_buy, acceptable_price= middle - self.ordering_thresh, trade_made="BUY", product=self.product, order_lst = orders, limit = self.limit)

                elif z_score > self.Z_THRESH:
                    sell_prices = StaticTrader.do_order_price(bot_orders = order_depth.buy_orders, operator = operator.gt, max_vol = max_sell, acceptable_price= middle + self.ordering_thresh, trade_made="SELL", product=self.product, order_lst = orders, limit = self.limit)
...
class BerriesMeanReversion():
        ...
        def mean_reverse_volume(self, expected_val_total, order_depth, buy_volume, sell_volume, orders):
            ...
            # TODO - FIX THIS THRESHS, ASK AMAL
...
class Trader:
    # inital paramters for round 2-
        # MeanReversion(window_size = 5, z_thresh = 1.5, product = "BANANAS")
    wrappers = {
        "PEARLS": [MeanReversion(window_size = 1000, z_thresh = 0, product = "PEARLS", ordering_thresh = 0, historical_mean=10000)],
