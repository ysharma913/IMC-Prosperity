        ...
        def make_orders(self, state):
            ...
            if len(self.rolling_window) >= self.WINDOW_SIZE:
                ...
                if z_score < -self.Z_THRESH:
                    buy_prices = StaticTrader.do_order_price(bot_orders = order_depth.sell_orders, operator = operator.lt, max_vol = max_buy, acceptable_price= middle - self.ordering_threshold, trade_made="BUY", product=self.product, order_lst = orders, limit = self.limit)

                elif z_score > self.Z_THRESH:
                    sell_prices = StaticTrader.do_order_price(bot_orders = order_depth.buy_orders, operator = operator.gt, max_vol = max_sell, acceptable_price= middle + self.ordering_threshold, trade_made="SELL", product=self.product, order_lst = orders, limit = self.limit)
...
class BerriesMeanReversion(MeanReversion):
