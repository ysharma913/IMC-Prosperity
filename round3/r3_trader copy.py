...
class StaticTrader:
    ...
    limits = {'PEARLS': 20, 'BANANAS': 20, 'COCONUTS': 600, 'PINA_COLADAS': 300, 'BERRIES': None, 'DIVING_GEAR': None}
...
class AsyncTrader: 
    ...
    def __init__(self, product, trend, window_size):
        self.rolling_window = list()
        self.product = product
        self.trend = trend
        self.WINDOW_SIZE = window_size
    ...
    def update_window(self):
        # We pop a value from the beginning of the trends and add a new value from the current iteration
        # Only done AFTER we place orders
        return
    ...
    def make_orders(self, state):
        ...
        window_offset = 50
        assert window_offset <= self.WINDOW_SIZE

        trend = self.delta(window_offset)
        # We want to trade based on the slope of the trend data at a previous time
        if trend > 0:
            # The slope is trending upwards
            StaticTrader.do_order_price(order_depth.sell_orders, operator.lt, max_buy_vol, current_price, 'BUY', self.product, orders, self.limit)
        elif trend < 0:
            # The slope is trending downwards
            StaticTrader.do_order_price(order_depth.buy_orders, operator.gt, max_sell_vol, current_price, 'SELL', self.product, orders, self.limit)
        else:
            StaticTrader.marketmake(self.product, 'BUY', 10, current_min_sell, max_sell_vol, orders)
            StaticTrader.marketmake(self.product, 'SELL', 10, current_max_buy, max_buy_vol, orders)
